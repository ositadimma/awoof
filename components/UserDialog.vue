<template>
  <div class="user-form-container" @click.self="closeModal">
    <div class="user-form-body animate__fadeIn">
      ...
      <div>Are you sure yo want to {{ detail }} this user?</div>
      <div class="btn-c">
        <span v-if="detail==='suspend'" class="btn-d suspend" @click="suspendUser">
          suspend
        </span>
        <span v-if="detail==='delete'" class="btn-d delete" @click="deleteUser">
          delete
        </span>
        <span v-if="detail==='reinstate'" class="btn-d delete" @click="reinstateUser">
          reinstate
        </span>
...
export default {
  ...
  methods: {
    ...
    async reinstateUser () {
      ...
      try {
        const response = await this.$axios.$get(
          `/users/reinstate_user/${this.userDetail._id}`
        )
        if (response) {
          this.$toast.global.custom_success('User Reinstated')
          this.$nuxt.refresh()
          // this.setDataToDefault()
          this.$store.commit('setModalOpen', false)
        }
    ...
    async deleteUser () {
      ...
      try {
        const response = await this.$axios.$get(
          `/users/delete_user/${this.userDetail._id}`
        )
        if (response) {
          this.$toast.global.custom_success('User Deleted')
