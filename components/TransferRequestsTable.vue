        ...
        <tbody v-show="data.length > 0">
          <tr v-for="(transfer, index) in paginatedData" :key="index">
            <td data-title="Trans Ref." class="Ref">
              <p>{{ transfer.user.firstName + ' ' + transfer.user.lastName }}</p>
            </td>
            ...
            <td data-title="Date Posted">
              ₦{{ transfer !== undefined ? amountDelimeter(transfer.transfer.amount) : amountDelimeter(0) }}
            </td>
            <td data-title="Date Posted">
              {{ transfer.type }}
            </td>
            <td data-title="Date Posted">
              {{ transfer !== undefined && transfer.transfer.pending
                ? 'Pending'
                : 'Successful'
              }}
            </td>
            <td data-title="Date Posted">
              {{ format_date(transfer.transfer.transaction_date) }}
            </td>
            ...
            <td class="View">
              <ArrowCircle
                @click.native="move(transfer.transfer._id,transfer.type)"
              />
            </td>
          </tr>
        </tbody>
...
export default {
  ...
  methods: {
    ...
    move (id, type) {
      localStorage.setItem('typeoftransferrequestawoof', type)
      this.$router.push(`/transferrequests/${id}`)
    },
    format_date (value) {
