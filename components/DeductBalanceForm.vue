  ...
  <div class="user-form-container" @click.self="closeModal">
    <div class="user-form-body animate__fadeIn">
      ...
      <div class="form">
        ...
        <input v-model="bankName" type="text" placeholder="Please Enter">
        <label>Account Name</label>
        <input v-model="accountName" type="text" placeholder="Please Enter">
        <label>Account Number</label>
        <input v-model="accountNumber" type="text" placeholder="Please Enter">

        <!-- <label>Referral ID</label>
        <input type="text" placeholder="Please Enter"> -->
        <button v-show="amount <= 0 && accountName == '' && bankName == '' && !loading" class="disable-1 btn-cmpt">
          Continue
        </button>
        <button
          v-show="amount > 0 && accountName != '' && bankName != '' && !loading"
          class="btn-cmpt"
          @click="deductUserBalance"
        >
          Continue
        </button>
    ...
    async deductUserBalance () {
      ...
      try {
        const response = await this.$axios.$post(
          `/admins/deduct_balance/${this.$route.params.id}`,
          {
